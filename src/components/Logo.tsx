...
export function BrandMark({ size = 40, detail = false, style }: { size?: number; detail?: boolean; style?: CSSProperties }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: radius, background: INK, display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none', ...style }}>
      {detail ? (
        <svg width={inner} height={inner} viewBox="0 0 100 100" fill="none" aria-hidden="true">
...
// Marchio + wordmark "Beach Diary" (per header, sidebar, splash).
export function BrandLockup({ size = 38, textSize = 18, gap = 12 }: { size?: number; textSize?: number; gap?: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap }}>
      <BrandMark size={size} />
      <span style={{ font: `700 ${textSize}px 'Space Grotesk'`, letterSpacing: '-.4px', color: INK, whiteSpace: 'nowrap' }}>
        Beach<span style={{ fontWeight: 400, color: 'rgba(27,42,74,.55)' }}> Diary</span>
      </span>
    </div>
  )
}
