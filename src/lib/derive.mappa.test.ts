...
// Variante con catalogo luoghi: serve solo ai test sulle coordinate salvate.
const mappaConLuoghi = (tornei: Partial<Tournament>[], venues: Venue[], fYear = 'Sempre') =>
...
describe('deriveMappa — i tre bucket, nessuno silenzioso', () => {
  ...
  it('una città vuota non è un pin: è contata in senzaCitta', () => {
    // Il luogo è facoltativo in tutte e tre le porte di creazione (form
    // completo, rapido e assistente): un torneo senza è un caso frequente, non
    // un errore.
  ...
  it('un luogo con coordinate ripesca una città che il gazetteer non conosce', () => {
    // Il caso che questa integrazione esiste per risolvere: hai preso la
    // posizione col GPS, quindi il punto lo SAPPIAMO — restare in "Non ancora
    // sulla mappa" sarebbe assurdo.
    const luogo = makeVenue({ id: 'v1', name: 'Bagno Fooburgo', city: 'Fooburgo', lat: 44.06, lng: 12.57 })
    const m = mappaConLuoghi([{ id: 'x', city: 'Fooburgo', venueId: 'v1' }], [luogo])
    expect(m.sconosciute).toHaveLength(0)
    expect(m.pins.map((p) => p.key)).toEqual(['fooburgo'])
  })

  it('senza coordinate il luogo non ripesca nulla: resta sconosciuta', () => {
    const luogo = makeVenue({ id: 'v1', name: 'Bagno Fooburgo', city: 'Fooburgo' })
    const m = mappaConLuoghi([{ city: 'Fooburgo', venueId: 'v1' }], [luogo])
    expect(m.pins).toHaveLength(0)
    expect(m.sconosciute.map((c) => c.city)).toEqual(['Fooburgo'])
  })

  it('le coordinate di un luogo NON spostano un pin che il gazetteer già colloca', () => {
    // L'invariante di sicurezza: le coordinate sono dati inseriti a mano e
    // possono essere sbagliate. Qui il luogo dice `0, 0` (Golfo di Guinea) ma
    // il pin di Rimini resta al suo posto — altrimenti un errore di battitura
    // farebbe sparire una città dalla mappa.
    const luogo = makeVenue({ id: 'v1', name: 'Bagno 26', city: 'Rimini', lat: 0, lng: 0 })
    const conLuogo = mappaConLuoghi([{ city: 'Rimini', venueId: 'v1' }], [luogo])
    const senzaLuogo = mappa([{ city: 'Rimini' }])
    expect(conLuogo.fuoriItalia).toHaveLength(0)
    expect(conLuogo.pins).toHaveLength(1)
    expect([conLuogo.pins[0].x, conLuogo.pins[0].y]).toEqual([senzaLuogo.pins[0].x, senzaLuogo.pins[0].y])
  })

  it('un luogo ripescato ma fuori dal riquadro finisce in "fuori Italia", non fra i pin', () => {
    const luogo = makeVenue({ id: 'v1', name: 'Platja', city: 'Fooburgo', lat: 38.9, lng: 1.43 })
    const m = mappaConLuoghi([{ city: 'Fooburgo', venueId: 'v1' }], [luogo])
    expect(m.pins).toHaveLength(0)
    expect(m.fuoriItalia.map((c) => c.city)).toEqual(['Fooburgo'])
  })

  it('due luoghi con coordinate nella stessa città danno sempre lo stesso pin', () => {
    // Determinismo: la scelta è per id, non per l'ordine in cui il DB
    // restituisce i tornei.
    const a = makeVenue({ id: 'v1', name: 'Bagno A', city: 'Fooburgo', lat: 44.06, lng: 12.57 })
    const b = makeVenue({ id: 'v2', name: 'Bagno B', city: 'Fooburgo', lat: 45.5, lng: 9.2 })
    const tornei = [
      { id: 't1', city: 'Fooburgo', venueId: 'v1', date: '2026-06-01' },
      { id: 't2', city: 'Fooburgo', venueId: 'v2', date: '2026-06-02' },
    ]
    const dritto = mappaConLuoghi(tornei, [a, b])
    const rovescio = mappaConLuoghi([...tornei].reverse(), [b, a])
    expect(dritto.pins).toHaveLength(1)
    expect([dritto.pins[0].x, dritto.pins[0].y]).toEqual([rovescio.pins[0].x, rovescio.pins[0].y])
  })
