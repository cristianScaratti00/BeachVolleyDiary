...
//
// Lavora su `DiaryData` e non su `TorneoCard` per tre motivi concreti:
// `TorneoCard` non espone `city` (è appiattita dentro la stringa `meta`, quattro
// punti di costruzione fra client e server); i tornei CONDIVISI da un socio
// collegato esistono solo nei dati client — la RPC `tornei_list` filtra su
// `user_id = auth.uid()`; e il catalogo dei luoghi (`data.venues`), da cui
// arrivano le coordinate vere, sta anch'esso solo lì.
//
// Il pin resta per CITTÀ, non per luogo: due bagni della stessa spiaggia sono un
// pin solo. A 1 unità di viewBox ≈ 3 km non c'è modo di distinguerli, e
// "conquistare Riccione" è la frase che questa mappa vuole dire. Il singolo
// luogo ha la sua mappa a zoom di strada nel dettaglio torneo (`VenueMap`).
// ============================================================================
import { computeStats, placementRank, fmtDate, yearOf } from './stats'
import { dotForRank, todayISO } from './derive'
import type { DiaryData, Tournament, Venue } from './models'
...
// ---------------------------------------------------------------------------
// Dove cade il pin: gazetteer, poi le coordinate del luogo
// ---------------------------------------------------------------------------
// Da quando il luogo è un'entità (`venues`), alcuni tornei portano coordinate
// VERE — prese dal GPS o incollate a mano nel VenuePicker. Sono l'unico modo di
// mettere sulla mappa una spiaggia che il gazetteer non conosce.
//
// ⚠️ L'ordine è gazetteer PRIMA, coordinate del luogo come RIPESCAGGIO — non il
// contrario, benché le seconde siano più precise. Il gazetteer è una tabella
// curata e stabile; le coordinate di un luogo sono dati inseriti dagli utenti e
// possono essere sbagliate (un `0, 0` incollato manda il pin nel Golfo di Guinea
// e la città sparisce in "fuori dai confini"). Così com'è, un pin già corretto
// non si sposta MAI per colpa di una posizione salvata male: le coordinate del
// luogo possono solo aggiungere pin, mai toglierne o spostarne.
//
// A questa scala la maggiore precisione non si vedrebbe comunque: 1 unità di
// viewBox ≈ 3 km, e il declustering tiene i pin a 13 unità l'uno dall'altro.
function coordinateDelLuogo(data: DiaryData, g: Gruppo): GeoPoint | null {
  const conCoordinate = g.tornei
    .map((t) => (t.venueId ? data.venues.find((v) => v.id === t.venueId) : undefined))
    .filter((v): v is Venue => !!v && v.lat != null && v.lng != null)
    // Ordinamento per id: due luoghi con coordinate nella stessa città danno
    // sempre lo stesso pin, comunque il DB abbia ordinato i tornei.
    .sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
  const v = conCoordinate[0]
  return v ? { lat: v.lat as number, lng: v.lng as number } : null
}

function puntoDelGruppo(data: DiaryData, g: Gruppo): GeoPoint | null {
  return geocodeCity(g.tornei[0].city) ?? coordinateDelLuogo(data, g)
}
...
export function deriveMappa(data: DiaryData, fYear: string, today = todayISO()): MappaData {
  ...
  const semi = raggruppaPerCitta(giocati)
    .map((g) => ({ key: g.key, punto: puntoDelGruppo(data, g) }))
    .filter((s): s is { key: string; punto: GeoPoint } => !!s.punto)
  ...
  for (const g of raggruppaPerCitta(delPeriodo)) {
    const punto = puntoDelGruppo(data, g)
    if (!punto) {
      // Né il gazetteer né un luogo con coordinate. NON sparisce: la sezione
      // "Non ancora sulla mappa" la rende visibile, ed è così che si scopre cosa
      // manca — al gazetteer, o alla scheda del luogo.
